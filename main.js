...
function InterplotCinematic() {
  ...
  case 1:
    ...
    var nemesis = NamedMonster(GetI(Traits,'Tier')+3);
...
function NamedMonster(tier) {
  var lev = 0;
  var result = '';
  for (var i = 0; i < 5; ++i) {
    var m = Pick(K.Monsters);
    if (!result || (Abs(tier-StrToInt(Split(m,1))) < Abs(tier-lev))) {
      result = Split(m,0);
...
function MonsterTask(tier) {
  ...
  if (tier < 1) tier = 1;
  // tier = tier of puissance of opponent(s) we'll return
  ...
  if (tier-lev > 10) {
    ...
    qty = Math.floor((tier + Random(lev)) / Max(lev,1));
    if (qty < 1) qty = 1;
    tier = Math.floor(tier / qty);
  }
  ...
  if ((tier - lev) <= -10) {
    ...
    i = 10+(tier-lev);
    i = 5-Random(i+1);
    result = Sick(i,Young((lev-tier)-i,result));
  } else if (((tier-lev) < 0) && (Random(2) == 1)) {
    result = Sick(tier-lev,result);
  } else if (((tier-lev) < 0)) {
    result = Young(tier-lev,result);
  } else if ((tier-lev) >= 10) {
    result = 'messianic ' + result;
  } else if ((tier-lev) > 5) {
    i = 10-(tier-lev);
    i = 5-Random(i+1);
    result = Big(i,Special((tier-lev)-i,result));
  } else if (((tier-lev) > 0) && (Random(2) == 1)) {
    result = Big(tier-lev,result);
  } else if (((tier-lev) > 0)) {
    result = Special(tier-lev,result);
  }
  ...
  return { 'description': result, 'tier': tier };
}
...
function EquipPrice() {
  return  5 * GetI(Traits,'Tier') * GetI(Traits,'Tier') +
    10 * GetI(Traits,'Tier') +
    20;
}
...
function Dequeue() {
  while (TaskDone()) {
    if (Split(game.task,0) == 'kill') {
      ...
      // buy some equipment
      Add(Inventory,'Grist',-EquipPrice());
      WinEquip();
    } else if ((game.task == 'market') || (game.task == 'sell')) {
      if (game.task == 'sell') {
        var amt = GetI(Inventory, 1) * GetI(Traits,'Tier');
        if (Pos(' of ', Inventory.label(1)) > 0)
          amt *= (1+RandomLow(10)) * (1+RandomLow(GetI(Traits,'Tier')));
        Inventory.remove1();
        Add(Inventory, 'Grist', amt);
      }
    ...
    if (game.queue.length > 0) {
      ...
      Task('Heading to the local crocodile markets',4 * 1000);
      game.task = 'market';
    } else if ((Pos('kill|',old) <= 0) && (old != 'heading')) {
      if (GetI(Inventory, 'Grist') > EquipPrice()) {
        Task('Alchemizing better equipment', 5 * 1000);
        game.task = 'buying';
      } else {
        Task('Heading out to adventure', 4 * 1000);
        game.task = 'heading';
      }
    } else {
      var nn = GetI(Traits, 'Tier');
      var t = MonsterTask(nn);
      var InventoryLabelAlsoGameStyleTag = 3;
      nn = Math.floor((2 * InventoryLabelAlsoGameStyleTag * t.tier * 1000) / nn);
...
function ListBox(id, columns, fixedkeys) {
  ...
  this.PutUI = function (key, value) {
    ...
    if (key == 'Visited' || key == 'Current') {
      item.children().last().html(value);
    } else {
...
function WinSpell() {
  AddR(Spells, K.Spells[RandomLow(Min(GetI(Stats,'WIS')+GetI(Traits,'Tier'),
                                      K.Spells.length))], 1);
}
...
function WinEquip() {
  ...
  var name = LPick(stuff, GetI(Traits,'Tier'));
  var qual = StrToInt(Split(name,1));
  name = Split(name,0);
  var plus = GetI(Traits,'Tier') - qual;
...
function NewTitle() {
  var posn = Random(K.TitleBits.length);
  var divider = Random(K.Dividers.length);
  var posn2 = Random(K.TitleBits.length);
  while (posn == posn2) {
    var posn2 = Random(K.TitleBits.length);
  }
  var newTitle = K.TitleBits[posn] + K.Dividers[divider] + K.TitleBits[posn2];
  Put(Traits, 4, newTitle);
}
...
function CompleteAct() {
  ...
  if (game.act > 1) {
    ...
    var newPlanet = 'Land of ' + K.PlanetAspect[posn] + " and " + K.PlanetAspect[posn2];
    Put(Planets, 0, newPlanet);
    Put(Planets, 1, game.Planets.Visited + '<p>' + newPlanet + '</p>');    
  }
  Brag('act');
}
...
function LevelUp() {
  ...
  WinSpell();
  NewTitle();
  ExpBar.reset(LevelUpTime(GetI(Traits,'Tier')));
  Brag('tier');
}
...
function FormCreate() {
  ExpBar =   new ProgressBar("ExpBar", "$remaining XP needed for next tier");
  EncumBar = new ProgressBar("EncumBar", "$position/$max captchalogue cards");
  ...
  Planets = new ListBox("Planets", 2, K.Planets);
  ...
  AllLists = [Traits,Stats,Spells,Equips,Inventory,Planets,Plots,Quests];
...
*/
  ...
  if (document) {
    var title = "Progress Stuck - " + GameSaveName();
    $("#title").text(title);
  ...
  if (!game.elapsed)
    Brag('start');
  StartTimer();
    $('body').css("background-image", "url("+game.backgroundKid+")");   
}
...
function changeKid(kid) {
  if (game.backgroundKid != kid) {
    $('body').css("background-image", "url(back_"+kid+".jpg)");      
    game.backgroundKid = 'back_'+kid+'.jpg';
  }
}
